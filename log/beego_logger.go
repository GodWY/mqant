...
func NewBeegoLogger(debug bool, ProcessID string, Logdir string, settings map[string]interface{}) *logs.BeeLogger {
	...
	if f, ok := settings["file"]; ok {
		...
		Service := "mqant"
		if suffix, ok := ff["service"]; ok {
			Service = suffix.(string)
		}
		ff["filename"] = fmt.Sprintf("%s/%v%s%s%s", Logdir, Prefix, ProcessID, Service, Suffix)
...
func NewLogger(options *Options, dir string, settings map[string]interface{}) *logs.BeeLogger {
	...
	if f, ok := settings["multifile"]; ok {
		...
		multifile["filename"] = fmt.Sprintf("%s/%v%s%s%s", logDir, Prefix, processId, service, Suffix)
		config, err := json.Marshal(multifile)
		if err != nil {
			logs.Error(err)
		}
